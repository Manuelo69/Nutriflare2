<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Maneja y cierra tus sesiones activas en otros navegadores') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Si es necesario, puede cerrar sesión en todas las demás sesiones de su navegador en todos sus dispositivos. Algunas de sus sesiones recientes se enumeran a continuación; sin embargo, esta lista puede no ser exhaustiva. Si crees que tu cuenta ha sido comprometida, también debe actualizar su contraseña.') }}
        </div>

        @if (count($this->sessions) > 0)
            <div class="mt-5 space-y-6 border border-black bg-white rounded-xl p-5">
                        ...
                        <div class="ms-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @else
                                        {{ __('Ultima activa') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
